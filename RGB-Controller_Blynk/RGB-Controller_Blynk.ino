...
char api_token[33];
char rgb_changed[] = "540";     //Colorcode, check README.txt for the Codes, it is changeable via the WiFi configuration
char ssid[40];
char pass[40];
...
//deafault delay values for the colorfunctions
...
void push(int bright_temp = brightness) {
  ...
  Blynk.run();
#ifdef DEBUG
  Serial.printf("\nRGB-Values: %d %d %d | %d % Brightness | %d ONOFF", R, G, B, bright_temp, onoff);
#endif
}
...
#ifdef RGBSLIDERCODE   
// codepart used for custom RGB Sliders in the Blynk app
// Button V2
// Set the Sliders on a range of 0 to 255
// RED: V3    GREEN: V4      BLUE: V5
// "send on release" is our recommendation
    ...
    if (SPIFFS.exists("/config1.json")) {
      ...
      if (configFile) {
        ...
        if (json.success()) {
          ...
          char *p2 = &rgb_changed[1];
          char *p3 = &rgb_changed[2];
...
// check, if some values have changed while a colorfunction is running
...
void setup() {
  ...
  //turn the RGB Strip white on startup
  R = 1023; G = 1023; B = 1023;
  brightness = 100;
  user_delay = 10;
  push();
  ...
  if (drd.detect()) {
    ...
    WiFiManagerParameter custom_api_token("api", "Blynk Auth Token", "", 33);
    WiFiManagerParameter rgb_change_values("rgbpins", "Pin Order/Colorcode (optional)", rgb_changed, 4);
